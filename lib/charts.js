...
// to redirect to the aliased packages. Node caches resolved dependency paths,
// so we hook Module._load to force the correct chart.js version on each reload.
(function registerSankeyPlugin() {
  ...
  ['chart.js-v3', 'chart.js-v4'].forEach((prefix, i) => {
    if (i > 0) {
      delete require.cache[sankeyPath];
    }
    Module._resolveFilename = function(request, parent, ...args) {
      if (request === 'chart.js') {
        return origResolveFilename.call(this, prefix, parent, ...args);
      }
    ...
    // Hook _load so that when the sankey module re-evaluates, its require('chart.js')
    // returns the correct version instead of hitting the stale cached resolution.
...
async function getChartJsForVersion(version) {
  if (version && version.startsWith('4')) {
    return require('chart.js-v4/auto').Chart;
  }
...
async function renderChartJs(
  ...
  if (chart.type === 'gauge') {
    chart.type = 'radialGauge';
  }

  // radialGauge plugin only supports Chart.js v2; force version override
  if (chart.type === 'radialGauge' && version && !version.startsWith('2')) {
    version = '2.9.4';
  }
  ...
  // horizontalBar was removed in Chart.js v3+; convert to bar with indexAxis: 'y'
  // This must run after progressBar→horizontalBar conversion above.
  if (
    chart.type === 'horizontalBar' &&
    version &&
    (version.startsWith('3') || version.startsWith('4'))
  ) {
    chart.type = 'bar';
    chart.options = chart.options || {};
  ...
  if (version.startsWith('3') || version.startsWith('4')) {
    ...
    // Sankey is pre-registered at startup for both v3 and v4 — no per-request loading needed.
  }
  if (!chart.plugins) {
    if (version.startsWith('3') || version.startsWith('4')) {
      chart.plugins = [];
